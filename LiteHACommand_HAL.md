...
| **附加消息长度** | 15字节       |                                          |
| *附加消息序号*   | *附加名称*     | *附加值*                                    |
| 0          | 操作类型       | 1字节，0为添加场景，1为修改场景名称                      |
| 1-4        | 地理分区       | 4字节，详情请看LiteHAProtol_HAL里面的地理分区列表，只有添加场景才有效 |
| 5-14       | 场景名称       | 10字节，输入的名称内容，最多10字节                      |
...
| *附加消息序号*   | *附加名称*     | *附加值*               |
| 0          | 状态位        | 1字节，0xc5成功，0xc8失败   |
| 1          | 操作位        | 1字节，0为添加场景，1为修改场景名称 |



##### 设置场景内容

场景设置，每增加或减少一个设备动作，都是直接操作设备，成功后才保存到数据库，不成功就进行提示，在手机端进行操作，也可以通过网关网页端进行操作，但是网页端操作是直接操作网关的数据库和直接发指令给设备进行场景登记。
